...
class NPSDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        Y_c_polar_gt, Y_d_polar_gt = compute_polar_gt_masks(r_c_gt, r_d_gt, self.n_rho)
...
def load_csv():
    ...
    if n_before != n_after:
        print(
            f"[dataset] Filtered {n_before - n_after} rows with missing files. "
            f"Remaining: {n_after}"
        )
...
def get_external_dataloader(csv_path, batch_size=4, num_workers=4):
    ...
    print(
        f"[dataset] External CSV: {len(df)} samples from {os.path.basename(csv_path)}"
    )
