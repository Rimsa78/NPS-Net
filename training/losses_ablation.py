...
Two loss modes:
    ...
    2. AblationLossStaged  — for B4: full 7-term staged loss
       Identical to ThreeSixty/losses.py NPSCombinedLoss.
...
from config import (
    LAMBDA_CART, LAMBDA_POLAR, LAMBDA_RIM,
    LAMBDA_DIST, LAMBDA_SHAPE, LAMBDA_CONS, LAMBDA_SMOOTH,
    SMOOTHL1_DELTA, STAGE_A_END, STAGE_B_END,
)


# ==============================================================================
# BUILDING BLOCKS (identical to ThreeSixty/losses.py)
# ==============================================================================
...
class AblationLossStageA(nn.Module):
    ...
    def forward(self, outputs, batch, epoch=0):
        ...
        l_cart = self.cart_loss(
            outputs['Y_c_cart'], outputs['Y_d_cart'],
            batch['cup_mask'], batch['disc_mask'],
        )

        l_polar = self.polar_loss(
            outputs['P_c_polar'], outputs['P_d_polar'],
            batch['Y_c_polar_gt'], batch['Y_d_polar_gt'],
        )

        l_rim = self.rim_loss(
            outputs['r_c_m'], outputs['r_d_m'],
            batch['r_c_gt'], batch['r_d_gt'],
        )

        total = (LAMBDA_CART  * l_cart
               + LAMBDA_POLAR * l_polar
               + LAMBDA_RIM   * l_rim)
    ...
    def forward(self, outputs, batch, epoch=0):
        ...
        if epoch >= STAGE_A_END:
            l_dist = self.dist_loss(
                outputs['p_d_s'], outputs['p_alpha_s'],
                batch['q_d'], batch['q_alpha'],
            )
            l_shape = self.shape_loss(
                outputs['r_d_s'], outputs['r_c_s'],
                batch['r_d_gt'], batch['r_c_gt'],
            )
            l_smooth = self.smooth_loss(
                outputs['r_d_s'], outputs['r_c_s'],
            )

            total = total + (LAMBDA_DIST   * l_dist
        ...
        if epoch >= STAGE_B_END:
            l_cons = self.cons_loss(
                outputs['r_d_m'], outputs['r_c_m'],
                outputs['r_d_s'], outputs['r_c_s'],
                outputs['gamma_d'], outputs['gamma_c'],
            )
