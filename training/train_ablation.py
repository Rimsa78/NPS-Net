...
from config import (
    BATCH_SIZE, IMAGE_SIZE, DEVICE, NUM_WORKERS, SEED,
    NUM_EPOCHS, MAX_LR, N_THETA, N_RHO,
    WEIGHT_DECAY, GRAD_CLIP_MAX_NORM,
    STAGE_A_END, STAGE_B_END,
    get_checkpoint_dir,
)
...
def compute_vcdr(cup_mask_2d, disc_mask_2d):
    ...
    if disc_rows.numel() == 0:
        return 0.0
    cup_h = (cup_rows.max() - cup_rows.min() + 1).float() if cup_rows.numel() > 0 else torch.tensor(0.0)
...
def train_one_epoch(model, loader, criterion, optimizer, scaler,
                    epoch, total_epochs, device, batch_scheduler=None):
    model.train()
...
def _load_resume_checkpoint(device, ckpt_dir):
    ...
    if os.path.isfile(path):
        ...
        print(f"  Loaded resume checkpoint: epoch={ckpt['epoch']}, "
...
def train(variant, resume=False):
    ...
    if resume:
        ...
        if resume_ckpt is not None:
            ...
            print(f"  Resuming from epoch {start_epoch}, best_cup_dice={best_cup_dice:.4f}")
        else:
            print("  No resume checkpoint found, starting from scratch.")

    if HAS_WANDB:
        wandb.init(project="NPS_Net_Ablation", config={
            'variant': variant,
    ...
    for epoch in range(start_epoch, NUM_EPOCHS):
        ...
        train_metrics = train_one_epoch(
            ...
            epoch=epoch, total_epochs=NUM_EPOCHS, device=device,
        ...
        if val_metrics['cup_dice'] > best_cup_dice:
            ...
            _save_checkpoint(model, optimizer, epoch, best_cup_dice,
                            path=os.path.join(ckpt_dir, 'best_model.pth'))

        _save_resume_checkpoint(model, optimizer, epoch,
                                 best_dice=best_cup_dice, scaler=scaler,
                                 ckpt_dir=ckpt_dir)

    _save_checkpoint(model, optimizer, NUM_EPOCHS - 1, best_cup_dice,
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='NPS-Net Ablation — Training')
