"""
Prepare the cropped REFUGE dataset for the baseline inference pipeline.

Steps:
    1. Split each cropped mask (.bmp) into separate disc and cup binary masks (.png)
    2. Compute vertical CDR from the masks
    3. Generate a CSV file (Raw, Disk, Cup, CDR, Type) compatible with
       Comparision/dataset.py

Mask encoding (REFUGE):
    255 = background
    128 = optic disc (rim)
      0 = optic cup
...
Output:
    Dataset/REFUGE/cropped_disc_masks/   — binary disc masks (white on black)
    Dataset/REFUGE/cropped_cup_masks/    — binary cup masks (white on black)
    Map/Corrected_refuge.csv             — CSV index file
"""
...
CSV_PATH = os.path.join("Map", "Corrected_refuge.csv")
...
def main():
    mask_paths = sorted(glob.glob(os.path.join(CROPPED_MASKS, "*.bmp")))
