...
def compute_self_score(out, inside_circle_dev):
    ...
    score = W_MASS * disc_mass + W_SHARPNESS * sharpness + W_COMPACT * compactness
    return score
...
@torch.no_grad()
def evaluate_polar_tta(
    model, loader, device, dataset_name="Test", hypotheses=None, top_k=DEFAULT_TOP_K
):
    model.eval()
    ...
    for batch in loader:
        ...
        for i in range(B):
            ...
            all_4sector_mae.append(
                np.mean([abs(sectors_pred[s] - sectors_gt[s]) for s in sectors_pred])
            )
...
def load_model(checkpoint_path=None):
    ...
    if not os.path.isfile(checkpoint_path):
        raise FileNotFoundError(
            f"B4 checkpoint not found: {checkpoint_path}\n"
    ...
    if "model_state_dict" in state:
        ...
        print(
            f"[B5/Polar-TTA] Loaded B4 checkpoint  epoch={epoch}  cup_dice={cup_dice}"
        )
    else:
...
def main():
    parser = argparse.ArgumentParser(description="Ablation B5 — Polar-TTA on B4 model")
    parser.add_argument(
    ...
    parser.add_argument(
        "--offsets", type=str, default=",".join(str(o) for o in DEFAULT_OFFSETS_PX)
    )
    parser.add_argument(
        "--scales", type=str, default=",".join(str(s) for s in DEFAULT_SCALES)
    )
    ...
    if args.test:
        ...
        r = evaluate_polar_tta(
            model, loader, DEVICE, "Test Split", hypotheses, args.top_k
        )
        print_results(r)
    ...
    if args.csv:
        ...
        r = evaluate_polar_tta(model, loader, DEVICE, args.name, hypotheses, args.top_k)
        print_results(r)
        all_results.append(r)
    ...
    if args.all_external:
        ...
        for name, csv_path in ext_datasets:
            ...
            r = evaluate_polar_tta(model, loader, DEVICE, name, hypotheses, args.top_k)
            print_results(r)
            all_results.append(r)
    ...
    if len(all_results) > 1:
        print("\n" + "=" * 100)
        print(
            f"  SUMMARY — B5: Polar-TTA on B4  (hyps={len(hypotheses)}  top-K={args.top_k})"
        )
        print("=" * 100)
        header = (
