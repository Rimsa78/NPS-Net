#!/usr/bin/env python3
"""
Combined Inference — REFUGE + Dristi.

Runs each model on a unified dataset combining REFUGE and Dristi,
reporting standard segmentation and geometry-fidelity metrics.

Baseline models (from models/baselines/):
    vanilla, attunet, resunet, polar_unet, transunet, beal, dofe

NPS-Net (from models/nps_net/):
    NPSNet — confidence-gated monotone factorised polar network

Usage:
    python inference_combined.py --model vanilla
    python inference_combined.py --model npsnet
    python inference_combined.py --model all          # run all models
    python inference_combined.py --model all --save-vis
"""
...
from datasets.dataset import BaselineDataset

REFUGE_CSV = os.path.join(PROJECT_ROOT, "Map", "Corrected_refuge.csv")
DRISTI_CSV = os.path.join(PROJECT_ROOT, "Map", "Corrected_DristiTest.csv")
...
def save_visualisations(
    model, loader, device, output_dir, dataset_name, max_vis=16, model_type="baseline"
):
    model.eval()
...
def load_baseline_model(model_name, checkpoint_path=None):
    """Load a baseline model from checkpoints/baselines/."""
    ...
    if "model_state_dict" in state:
        ...
        print(
            f"[baseline] Loaded {model_name} (cup_dice={state.get('best_cup_dice', '?')})"
        )
    else:
...
def get_combined_dataloader(batch_size=4, num_workers=4):
    """Load REFUGE and Dristi CSVs, combine them, and return a DataLoader."""

    # Load REFUGE
    if not os.path.isfile(REFUGE_CSV):
        raise FileNotFoundError(f"REFUGE CSV not found: {REFUGE_CSV}")
    df_refuge = pd.read_csv(REFUGE_CSV)
    print(f"[dataset] Loaded REFUGE: {len(df_refuge)} samples")

    # Load Dristi
    if not os.path.isfile(DRISTI_CSV):
        raise FileNotFoundError(f"Dristi CSV not found: {DRISTI_CSV}")
    df_dristi = pd.read_csv(DRISTI_CSV)
    print(f"[dataset] Loaded Dristi: {len(df_dristi)} samples")

    # Combine
    df_combined = pd.concat([df_refuge, df_dristi], ignore_index=True)
    ...
    valid_mask = (
        df_combined["Raw"].apply(lambda x: os.path.isfile(str(x)))
        & df_combined["Disk"].apply(lambda x: os.path.isfile(str(x)))
        & df_combined["Cup"].apply(lambda x: os.path.isfile(str(x)))
    )
    n_before = len(df_combined)
    df_combined = df_combined[valid_mask].reset_index(drop=True)
    if len(df_combined) < n_before:
        print(f"[dataset] Filtered {n_before - len(df_combined)} missing rows")

    print(f"[dataset] Combined Total: {len(df_combined)} samples")

    ds = BaselineDataset(df_combined, is_train=False)
...
def main():
    parser = argparse.ArgumentParser(description="Combined Inference — REFUGE + Dristi")
    parser.add_argument(
        "--model", type=str, required=True, choices=ALL_MODELS + ["all"]
    )
    parser.add_argument(
        "--save-vis", action="store_true", help="Save visualisations of predictions"
    )
    parser.add_argument(
        "--max-vis", type=int, default=16, help="Max number of visualisations to save"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    parser.add_argument("--tta", action="store_true", help="Use Polar-TTA for NPS-Net")
    parser.add_argument(
        "--top-k",
        type=int,
    ...
    # Get combined dataloader once
    loader = get_combined_dataloader(BATCH_SIZE, NUM_WORKERS)

    for model_name in models_to_run:
        print(f"\n{'═' * 70}")
        print(f"  Combined Inference: {model_name}")
        print(f"{'═' * 70}")
        ...
        # Use Polar-TTA for NPSNet by default (disable with --no-tta if implemented, but here we use --tta flag)
        # Wait, inference_papila.py uses --no-tta to DISABLE.
        # My script uses --tta to ENABLE.
        # I should probably match inference_papila.py logic or stick to my flag.
        # inference_papila.py: use_tta = (model_type == 'npsnet' and not args.no_tta)
        # My script: use_polar_tta=(args.tta and model_type == 'npsnet')
        # I'll stick to my flag for simplicity, or change to match.
        # Let's use --tta to enable, as per my help message.

        use_tta = args.tta and model_type == "npsnet"
    ...
    if len(all_results) > 1:
        print(f"\n{'=' * 110}")
        print(f"  Combined SUMMARY — Cross-Domain Generalisation")
        print(f"{'=' * 110}")
        h = f"{'Model':<18} {'Cup Dice':>10} {'Disc Dice':>10} {'Cup HD95':>10} {'vCDR MAE':>10} {'Rim MAE':>10} {'Nest%':>8}"
        print(h)
        print("-" * 110)
        for r in all_results:
            name = r["dataset"].replace(" Combined", "")
            print(
