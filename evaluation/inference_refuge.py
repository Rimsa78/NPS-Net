#!/usr/bin/env python3
"""
REFUGE External-Set Inference — All baselines + NPS-Net.

Runs each model on the cropped REFUGE dataset and reports standard segmentation
...
Usage:
    python inference_refuge.py --model vanilla
    python inference_refuge.py --model npsnet
    python inference_refuge.py --model all          # run all models
    python inference_refuge.py --model all --save-vis
"""
...
def main():
    parser = argparse.ArgumentParser(description="REFUGE Inference — All Models")
    parser.add_argument(
        "--model",
        type=str,
        required=True,
    ...
    parser.add_argument(
        "--csv", type=str, default=REFUGE_CSV, help="Path to REFUGE CSV"
    )
    parser.add_argument(
    ...
    if not os.path.isfile(args.csv):
        print(f"[ERROR] REFUGE CSV not found: {args.csv}")
    ...
    for model_name in models_to_run:
        print(f"\n{'═' * 70}")
        print(f"  REFUGE inference: {model_name}")
        print(f"{'═' * 70}")
    ...
    if len(all_results) > 1:
        print(f"\n{'=' * 110}")
        print(f"  REFUGE SUMMARY — Cross-Domain Generalisation")
        print(f"{'=' * 110}")
        h = (
